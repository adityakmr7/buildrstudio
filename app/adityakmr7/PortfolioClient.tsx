...
const SKILLS = [
  ...
  {
    category: "AI / ML",
    items: ["LLM APIs (OpenAI/Gemini) — 85", "Prompt Engineering — 88", "RAG Pipelines — 80", "AI Agents — 75"],
  },
  {
    category: "Infrastructure",
    items: ["PostgreSQL / MongoDB — 86", "CI/CD Pipelines — 85", "AWS / Docker — 80", "Redis / Caching — 78"],
  },
];
...
const TICKER_ITEMS = ["React Native","Next.js","TypeScript","Node.js","GraphQL","PostgreSQL","AI/LLM APIs","AWS","RAG Pipelines","Prompt Engineering"];
...
export default function PortfolioClient() {
  ...
  return (
    <div className="ab">
      <style>{`
        .ab { min-height: 100vh; background: var(--bg); color: var(--text-1); font-family: var(--font); }
        .ab-inner { max-width: 780px; margin: 0 auto; padding: 80px 32px 120px; }
        .ab-hero { margin-bottom: 80px; }
        .ab-eyebrow { display: flex; align-items: center; gap: 8px; font-size: 11px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--text-3); margin-bottom: 24px; font-family: monospace; }
        .ab-eyebrow-sq { width: 10px; height: 10px; background: var(--fill); display: inline-block; flex-shrink: 0; }
        .ab-headline { font-size: clamp(48px, 8vw, 88px); font-weight: 900; line-height: 0.95; letter-spacing: -3px; text-transform: uppercase; margin: 0 0 40px; color: var(--text-1); }
        .ab-headline-hl { display: inline-block; background: var(--fill); color: var(--fill-text); padding: 0 8px; letter-spacing: -3px; }
        .ab-identity { display: flex; align-items: center; gap: 20px; margin-bottom: 40px; }
        .ab-avatar { width: 64px; height: 64px; border-radius: 50%; overflow: hidden; border: 2px solid var(--border); flex-shrink: 0; }
        .ab-bio { font-size: 15px; color: var(--text-2); line-height: 1.65; }
        .ab-spec { border: 1px solid var(--text-1); margin-bottom: 40px; }
        .ab-spec-header { background: var(--text-1); color: var(--bg); padding: 10px 16px; font-size: 11px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; font-family: monospace; }
        .ab-spec-row { display: grid; grid-template-columns: 100px 1fr; border-top: 1px solid var(--border); }
        .ab-spec-row:first-of-type { border-top: none; }
        .ab-spec-key { padding: 11px 16px; font-size: 11px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--text-3); font-family: monospace; border-right: 1px solid var(--border); }
        .ab-spec-val { padding: 11px 16px; font-size: 13px; color: var(--text-1); font-weight: 500; }
        .ab-ctas { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 80px; }
        .ab-btn { display: inline-flex; align-items: center; gap: 8px; padding: 14px 24px; font-size: 15px; font-weight: 700; letter-spacing: -0.2px; cursor: pointer; text-decoration: none; transition: opacity 0.15s; border: 1.5px solid var(--border-strong); border-radius: var(--r-md); font-family: var(--font); }
        ...
        .ab-section-label { display: flex; align-items: center; gap: 8px; font-size: 11px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--text-3); margin-bottom: 20px; font-family: monospace; }
        .ab-skills-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0; border: 1px solid var(--border); }
        ...
        .ab-skill-cat { font-size: 11px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; font-family: monospace; color: var(--fill); margin-bottom: 12px; }
        .ab-skill-item { display: flex; justify-content: space-between; align-items: center; padding: 6px 0; border-bottom: 1px solid var(--border); font-size: 13px; color: var(--text-2); }
        ...
        .ab-ach-row { display: grid; grid-template-columns: 1fr auto; gap: 16px; align-items: center; padding: 14px 20px; border-bottom: 1px solid var(--border); transition: background 0.12s; }
        ...
        .ab-ach-meta { font-size: 10px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; font-family: monospace; color: var(--fill); white-space: nowrap; }
        .ab-project { border: 1px solid var(--border); margin-bottom: -1px; padding: 24px; text-decoration: none; display: block; color: var(--text-1); transition: background 0.12s; }
        ...
        .ab-project-name { font-size: 18px; font-weight: 800; letter-spacing: -0.5px; border-bottom: 2px solid var(--fill); padding-bottom: 1px; display: inline-block; }
        .ab-project-status { font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; font-family: monospace; color: var(--text-3); white-space: nowrap; padding-top: 4px; }
        ...
        .ab-project-tag { font-size: 10px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; font-family: monospace; padding: 3px 8px; border: 1px solid var(--border); color: var(--text-3); }
        .ab-exp-list { border: 1px solid var(--border); }
        .ab-exp-row { display: grid; grid-template-columns: 160px 1fr; border-bottom: 1px solid var(--border); transition: background 0.12s; }
        .ab-exp-row:last-child { border-bottom: none; }
        .ab-exp-row:hover { background: var(--surface); }
        .ab-exp-period { padding: 16px 20px; font-size: 11px; font-weight: 700; color: var(--text-3); font-family: monospace; border-right: 1px solid var(--border); line-height: 1.5; }
        ...
        .ab-contact { border: 1px solid var(--text-1); padding: 48px 40px; text-align: center; }
        .ab-contact-headline { font-size: clamp(32px, 5vw, 52px); font-weight: 900; letter-spacing: -2px; text-transform: uppercase; line-height: 1; margin-bottom: 16px; }
        .ab-contact-sub { font-size: 14px; color: var(--text-2); line-height: 1.7; max-width: 440px; margin: 0 auto 32px; }
        .ab-ticker { overflow: hidden; border-top: 1px solid var(--border); border-bottom: 1px solid var(--border); background: var(--text-1); color: var(--bg); margin: 64px 0; padding: 12px 0; white-space: nowrap; }
        .ab-ticker-inner { display: inline-flex; gap: 48px; animation: ab-scroll 20s linear infinite; font-size: 12px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; font-family: monospace; }
      ...
      <div className="ab-inner">
        <section className="ab-hero">
          ...
          <div className="ab-identity">
            <div className="ab-avatar">
              <Image src="/aditya-avatar.png" alt="Aditya Kumar" width={64} height={64}
                style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 15%" }} priority />
            </div>
            <p className="ab-bio">
          ...
          <div className="ab-ctas">
            <a href={siteConfig.author.calUrl} target="_blank" rel="noopener noreferrer" className="ab-btn ab-btn-fill">Book a Call →</a>
            <button className="ab-btn ab-btn-outline" onClick={handleCopyEmail}>{copied ? "Copied!" : "Copy Email"}</button>
            <a href={siteConfig.author.githubUrl} target="_blank" rel="noopener noreferrer" className="ab-btn ab-btn-outline">GitHub</a>
            <a href={siteConfig.author.twitterUrl} target="_blank" rel="noopener noreferrer" className="ab-btn ab-btn-outline">X / Twitter</a>
            <a href={siteConfig.author.linkedinUrl} target="_blank" rel="noopener noreferrer" className="ab-btn ab-btn-outline">LinkedIn</a>
          </div>
        </section>
        ...
        <section className="ab-section">
          <div className="ab-section-label"><span className="ab-eyebrow-sq" />Stack Depth</div>
        ...
        <section className="ab-section">
          <div className="ab-section-label"><span className="ab-eyebrow-sq" />Proof, Not Adjectives</div>
        ...
        <section className="ab-section">
          <div className="ab-section-label"><span className="ab-eyebrow-sq" />What I&apos;ve Shipped</div>
          <div>
            {PROJECTS.map((p) => (
        ...
        <section className="ab-section">
          <div className="ab-section-label"><span className="ab-eyebrow-sq" />Career Timeline</div>
          <div className="ab-exp-list">
            {EXPERIENCE.map((e) => (
              <div className="ab-exp-row" key={e.company + e.role}>
                <div className="ab-exp-period">
                  {e.period.split("—").map((p, i) => <div key={i}>{p.trim()}</div>)}
                </div>
        ...
        <section>
          <div className="ab-contact">
            ...
            <div style={{ display: "flex", gap: "10px", justifyContent: "center", flexWrap: "wrap" }}>
              ...
              <button className="ab-btn ab-btn-outline" onClick={handleCopyEmail}>{copied ? "Copied!" : "Copy Email"}</button>
              <a href={siteConfig.author.linkedinUrl} target="_blank" rel="noopener noreferrer" className="ab-btn ab-btn-outline">LinkedIn</a>
              <a href={siteConfig.author.substackUrl} target="_blank" rel="noopener noreferrer" className="ab-btn ab-btn-outline">Substack</a>
            </div>
          </div>
        </section>
      </div>
